      ...
      <div className="overflow-hidden rounded-xl border border-slate-200">
        <table className="min-w-full text-sm">
          <thead className="border-b border-slate-200 bg-slate-50">
            <tr>
              <th className="px-4 py-3 text-left text-slate-600">실행 ID</th>
              <th className="px-4 py-3 text-left text-slate-600">상태</th>
              <th className="px-4 py-3 text-left text-slate-600">시작</th>
              <th className="px-4 py-3 text-left text-slate-600">트리거</th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-slate-100">
                ...
                <td className="px-4 py-3 text-slate-600">{row.triggeredBy ?? "-"}</td>
