...
export default async function AdminAlertBoardPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Alert Task 보드</h1>
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        <Link
          ...
          className="rounded-xl border border-slate-200 bg-white px-4 py-2 font-medium text-slate-800 shadow-sm hover:bg-slate-50"
        >
          경고 KPI (차트)
        </Link>
        <Link
      ...
      <Suspense fallback={<div className="text-sm text-slate-500">보드 로딩...</div>}>
        <AlertTaskBoard />
      </Suspense>
    </div>
  );
