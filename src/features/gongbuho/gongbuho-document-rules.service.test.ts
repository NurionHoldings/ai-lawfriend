...
describe("extractGongbuhoRuleStringArrays", () => {
  ...
  it("parses object-shaped rules generated by Legal Knowledge Pipeline", () => {
    const r = extractGongbuhoRuleStringArrays({
      validationRules: [{ text: "증거와 주장 연결을 확인" }],
      forbiddenRules: [
        {
          id: "FR-1",
          pattern: "승소 확정",
      ...
      expertReviewPoints: [{ title: "손해액 재검토", description: "산정 근거 확인" }],
    });

    expect(r.validationRules).toEqual(["증거와 주장 연결을 확인"]);
    expect(r.forbiddenRules[0]).toContain("승소 확정 표현 금지");
    expect(r.forbiddenRules[0]).toContain("승소 확정");
    expect(r.expertReviewPoints[0]).toContain("손해액 재검토");
  });
});
