...
export function buildCaseSummaryGenerateInstructions(mode: "AI_ENRICH" | "AI_REGENERATE") {
  ...
  return [
    ...
    "각 출력 문장과 제목에는 입력에 표시된 사용 가능한 출처 ref를 하나 이상 연결하세요. 존재하지 않는 ref를 만들지 마세요.",
    '스키마: {"caseOverview":string,"timeline":string[],"issues":string[],"riskNotes":string[],"checklist":string[],"contractSections":{heading:string,body:string}[]|optional,"grounding":{"claim":string,"sourceRefs":string[]}[]}',
  ].join("\n\n");
}
