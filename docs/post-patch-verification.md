...
## 18) 사건 패키지 공유 점검 (6.3~6.8)

사건 패키지 공유 기능은 `add-case-package-share` migration 적용 후 아래 순서로 점검합니다.

1. 의뢰인 계정에서 사건을 하나 선택하고 `/cases/[caseId]/share`에서 고유번호를 발급합니다.
2. 공유 상세 화면 `/cases/[caseId]/share/[shareId]`에서 상태가 `ACTIVE`인지, 범위/다운로드 정책이 의도대로 보이는지 확인합니다.
3. 변호사 계정에서 `/lawyer/case-packages/lookup`으로 이동해 고유번호 조회가 되는지 확인합니다.
4. 첨부파일이 있는 사건이면 첨부자료 목록 열람, 다운로드 허용/비허용, 다운로드 로그 적재를 함께 확인합니다.
5. 요약본 출력 허용 시 `사건 패키지 요약본 출력`이 다운로드 응답으로 내려오는지 확인합니다.
6. 의뢰인 공유 취소 후 동일 고유번호가 더 이상 조회되지 않는지 확인합니다.

현재 기준 운영 확인 결과:

- 6.3 migration 적용 완료
- 6.4 공유 생성 / 목록 / 상세 / 취소 확인 완료
- 6.5 변호사 고유번호 조회 / 취소 후 차단 확인 완료
- 6.6 첨부 다운로드 정책 허용 / 비허용 및 로그 적재 확인 완료
- 6.7 열람 / 다운로드 / 취소 로그 적재 확인 완료
- 6.8 요약본 출력 응답 및 로그 적재 확인 완료
