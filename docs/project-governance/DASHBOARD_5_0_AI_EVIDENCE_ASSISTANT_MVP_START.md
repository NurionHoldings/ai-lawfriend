...
5.1 구현 시 파일 구조는 아래를 1차 후보로 한다. **페이지 실경로**는 라우트 그룹에 따라 `(protected)/admin` 아래에 둔다.

- `src/app/(protected)/admin/qa-evidence/page.tsx` (**구현됨**)
- `src/components/admin/qa-evidence/qa-evidence-assistant-client.tsx` (**구현됨**)
- `src/app/api/admin/qa-evidence/analyze/route.ts` (**구현됨**)
- `src/lib/qa-evidence/qa-evidence-schema.ts` (**구현됨**)
- `src/lib/qa-evidence/qa-evidence-analyzer.ts` (**구현됨**)
- `src/lib/qa-evidence/qa-evidence-renderer.ts` (**구현됨**)
...
5.1 MVP에서는 아래 API 하나만 우선 검토한다. **구분** 값은 §16(`DASHBOARD_4_7_*`)과 동일하게 **구현됨 / 후보 / 설계잠금 / 보류 / 폐기** 중 하나로만 표기한다.

| Method | Path | 구분 | 목적 |
| --- | --- | --- | --- |
| POST | /api/admin/qa-evidence/analyze | 구현됨 (5.1) | QA 원문 분석 및 Markdown 초안 생성 |

**페이지 실경로:** `src/app/(protected)/admin/qa-evidence/page.tsx` — 브라우저 경로 `/admin/qa-evidence`. `src/app/admin/qa-evidence/...` 형태는 라우트 그룹과 맞지 않으므로 문서·증빙에서 인용하지 않는다.
