...
업로드된 프로젝트 기준으로 현재 AI법친은 이미 다음 기반을 갖추고 있다.
...
- CaseStatus: CREATED → INTAKE_PENDING → IN_INTERVIEW → INTERVIEW_DONE → DRAFTING → REVIEW_PENDING → APPROVED → DELIVERED → CLOSED 등
- 사건 생성 / 상세 / 상태 전이 API
- 사건 인터뷰 API
- 사건 요약 생성 API
- 첨부파일 업로드 / 다운로드 API
...
- 의뢰인 / 변호사 / 관리자 대시보드
- 관리자 감사로그 / 알림 / 운영 큐 구조
- AI Evidence Assistant 5.1 구현 기반

현재 구조상 6.0은 완전히 새로 만드는 기능이 아니라, 기존의 아래 흐름을 묶어 외부 변호사 검토용 사건 패키지로 확장하는 것이다.
...
따라서 6.0에서 필요한 것은 사건을 새로 관리하는 구조가 아니라, 기존 Case를 바탕으로 공유 가능한 사건 패키지 레이어를 추가하는 것이다.

## 3. 일반인이 로그인해서 얻는 가치

일반 사용자가 AI법친을 통해 얻는 가치는 다음과 같이 명확히 정리해야 한다.
...
6.0의 핵심 기능명은 아래처럼 잡는다.

### 기능명

AI법친 사건 패키지

사건 패키지는 다음 정보를 하나로 묶은 변호사 검토용 단위다.
...
핵심은 법률문서 완성본이 아니라 변호사 검토용 사건 정리 패키지다.
...
- 변호사 로그인 + 고유번호 + 의뢰인 공유 승인 상태가 모두 맞아야 열람 가능해야 한다.
...
6.0은 개인정보와 민감 사건자료를 다루므로 아래는 필수다.
...
## 11. 기존 코드 기준 변경 필요 파일

기존 구조 기준으로 6.0에서 영향을 받을 파일 후보는 다음과 같다.
...
5.x에서는 DB 변경을 조심했지만, 6.x 사건 패키지는 결국 DB 모델이 필요하다.
...
주의: 6.0에서는 이 모델을 후보로만 둔다. 실제 Prisma 변경은 6.2 또는 6.3에서 진행하는 것이 안전하다.
...
사건 경위, 주요 날짜, 금액, 당사자, 피해 내용, 증거자료를
...
이 6.0은 AI법친의 핵심 사업가치 기능으로 봐야 한다.

### 5.x

AI Evidence Assistant — 운영 증빙 자동화 보조

### 6.x

사건 패키지 / 고유번호 공유 — 일반 사용자와 변호사를 연결하는 핵심 서비스 구조